...
func TestRepository_WithTX(t *testing.T) {
	...
	t.Run("Error", func(t *testing.T) {
		...
		err := r.WithTX(testCtx, func(ctx context.Context) error {
			...
			assert.Nil(t, err)
			return errors.Internal.Err(testCtx, "error")
		})
...
func TestRepository_FindOne_FindList(t *testing.T) {
	...
	t.Run("FindOne", func(t *testing.T) {
		...
		t.Run("With empty result", func(t *testing.T) {
			...
			assert.True(t, errors.NotFound.IsTyped(err))
		})
	})
...
func TestRepository_ErrorStackTrace(t *testing.T) {
	dbc := db.FromContext(testCtx)
	rec := Agent{ID: 111999, Name: "test"}
	err := dbc.Model(&rec).WherePK().Select()
	assert.NotNil(t, err)
	appErr := errors.Convert(testCtx, err, pgerr.Converter())
	assert.NotNil(t, appErr)
	appErrStr := fmt.Sprintf("%+v\n", appErr)
	assert.Contains(t, appErrStr, "---------------------------------")
}
