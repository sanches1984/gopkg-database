...
const unknownTableName = "unknown"
...
// TryFromContext returns the DB value stored in ctx
func TryFromContext(ctx context.Context) IClient {
	client, ok := ctx.Value(&dbKey).(IClient)
	if !ok {
		return nil
	}
...
func fromContextLogger(ctx context.Context) IDBLogger {
	logger, ok := ctx.Value(&dbLoggerKey).(IDBLogger)
	if !ok {
		return nil
	}
	return logger
...
// Log turn on logging of db queries
func Log(ctx context.Context) {
	logger := fromContextLogger(ctx)
	if logger != nil {
		logger.SetDuration(0)
		return
	}
	dbc := FromContext(ctx)
	if dbc == nil {
...
// Log turn on logging of long db queries
func LogLong(ctx context.Context, duration time.Duration) {
	logger := fromContextLogger(ctx)
	if logger != nil {
	...
	dbc.Db().AddQueryHook(&dbLogger{duration: duration})
}
